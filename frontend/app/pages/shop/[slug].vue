...
<template>
  <div class="container-shell py-8 sm:py-12">
    <div class="mb-7 text-xs text-stone-500"><NuxtLink to="/">{{ t('nav.home') }}</NuxtLink> / <NuxtLink to="/shop">{{ t('nav.shop') }}</NuxtLink> / {{ product?.name || t('product.cookie') }}</div>
    ...
    <div v-if="product" class="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-14">
      <div>
        ...
        <div class="mt-3 flex gap-3">
          <button class="h-16 w-16 overflow-hidden rounded-lg border-2 border-[#a85f4c]" :aria-label="t('product.viewMainPhoto')">
      ...
      <section class="self-center">
        ...
        <div class="mt-6 grid gap-3 text-sm">
          <p><strong>{{ t('product.ingredients') }}:</strong> <span class="text-stone-600">{{ product.ingredients }}</span></p>
          <p><strong>{{ t('product.allergens') }}:</strong> <span class="text-stone-600">{{ product.allergens }}</span></p>
          <p class="font-semibold" :class="product.in_stock ? 'text-[#78825e]' : 'text-red-700'">● {{ product.in_stock ? t('product.availableForPreorder') : t('product.currentlyUnavailable') }}</p>
        </div>

        <fieldset v-if="product.variants.length" class="mt-7">
          <legend class="text-xs font-bold uppercase tracking-wider">{{ t('product.choosePackSize') }}</legend>
        ...
        <div class="mt-7 flex flex-wrap gap-3">
          ...
          <button class="btn-primary min-w-52 flex-1" :disabled="!product.in_stock" @click="addToCart">{{ added ? t('product.addedToCart') : t('product.addToCart') }}</button>
        </div>
        <button class="btn-secondary mt-3 w-full" :disabled="!product.in_stock" @click="buyNow">{{ t('product.buyNow') }}</button>
        <div class="mt-8"><SharedTrustStrip /></div>
      </section>
    </div>

    <div v-else class="paper-card py-20 text-center">
      <h1 class="font-serif text-3xl">{{ t('product.notFound') }}</h1>
      <NuxtLink to="/shop" class="btn-primary mt-6">{{ t('product.returnToShop') }}</NuxtLink>
    </div>

    <section v-if="related.length" class="mt-20">
      <div class="section-heading"><h2>{{ t('product.youMayAlsoLike') }}</h2></div>
